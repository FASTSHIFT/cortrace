...
public:
    explicit OpenCsdDecoder(const EtmV4Config& cfg)
    {
        ...
        if (ocsd_dt_create_decoder(tree_, OCSD_BUILTIN_DCD_ETMV4I,
                OCSD_CREATE_FLG_FULL_DECODER, &ocfg, &csid)
            != OCSD_OK) {
            teardown();
            return;
        }
    ...
    bool process(const uint8_t* data, std::size_t len) override
    {
        ...
        while (consumed < len) {
            uint32_t used = 0;
            ocsd_datapath_resp_t r = ocsd_dt_process_data(tree_, OCSD_OP_DATA,
                index_ + consumed, static_cast<uint32_t>(len - consumed),
                data + consumed, &used);
            consumed += used;
...
private:
    ...
    void on_element(ocsd_trc_index_t idx, const ocsd_generic_trace_elem* elem)
    {
        ...
        case OCSD_GEN_TRC_ELEM_INSTR_RANGE:
            emit(Element::instr_range(static_cast<uint32_t>(elem->st_addr),
                ...
                classify(elem->last_i_type, elem->last_i_subtype),
                elem->last_instr_exec != 0, bidx));
            break;
    ...
    static ocsd_datapath_resp_t elem_trampoline(const void* ctx,
        const ocsd_trc_index_t idx_sop, const uint8_t /*cs_id*/,
